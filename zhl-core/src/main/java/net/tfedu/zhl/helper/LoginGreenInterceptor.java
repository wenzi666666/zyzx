 ...
 * @author wangwr
 *
 * @version create 1.0 update 2.0 2017-01-05 兼容url中传递参数的方式
 * 
 *
 *
 */
public class LoginGreenInterceptor implements HandlerInterceptor {
	...
	Logger logger = LoggerFactory.getLogger(LoginGreenInterceptor.class);
		...
		if (!StringUtils.isEmpty(token)) {
			...
			if (us != null) {
				...
				if (record != null && record.getUserid() > 0) {
					...
					UserSimple _us = userService.getUserSimpleById(record.getUserid(), model, config.getIsRepeatLogin(),
							false);
					// 重置登录时间
