...
public class zhldowncenter {
	 ...
	 *            下载中心地址
	 */
	public zhldowncenter(String CustomerID, String CustomerKey,
			String DownloadCenterURL) {
		this.CustomerID = CustomerID;
		this.CustomerKey = CustomerKey;
		this.DownloadCenterURL = DownloadCenterURL;
		if (this.DownloadCenterURL.lastIndexOf('/') != this.DownloadCenterURL
				.length() - 1)
			this.DownloadCenterURL += "/";
	}

	private String appendParam(String returnStr, String paramId,
			String paramValue) {
	...
	// 签名和生成请求URL功能函数
	private String GetSignCode(String strFileName, String strSerialNumber,
			String strCustomerID, String strCustomerKey) {
		// 验证签名
	...
	private String GetQueryString(String strFileName, String strSerialNumber,
			String strSignCode, String strCustomerID) {
		// 验证签名
		String SignString = appendParam("", "uid", strCustomerID);
		try {
			SignString = appendParam(SignString, "file", URLEncoder.encode(
					Base64.encode(strFileName.getBytes("utf-8")), "utf-8"));
		} catch (Exception e) {
		}
	...
	private String GetApplySignCode(String strFileName, String strCustomerID,
			String strCustomerKey) {
		return GetApplySignCode(null, strFileName, strCustomerID,
				strCustomerKey);
	}

	private String GetApplySignCode(String appMode, String strFileName,
			String strCustomerID, String strCustomerKey) {
		// 验证签名
		String SignString = appendParam("", "uid", strCustomerID);
		SignString = appendParam(SignString, "file", strFileName);
	...
	private String GetApplyQueryString(String strFileName, String strSignCode,
			String strCustomerID) {
		return GetApplyQueryString(null, strFileName, strSignCode,
				strCustomerID);
	}

	private String GetApplyQueryString(String appMode, String strFileName,
			String strSignCode, String strCustomerID) {
		// 验证签名
		String SignString = appendParam("", "uid", strCustomerID);
		try {
	...
	private String GetFilenameSign(String strSerialNumber, String strType) {
		...
		return strSerialNumber + "-" + strType + signchar[0] % 10 + signchar[1]
				% 10;
	}

	// 上传部分
	private String GetUploadSignCode(String strFilePath,
			String strSerialNumber, String strExtend, String strReturn) {
		// 验证签名
	...
	private String GetUploadQueryString(String strFilePath,
			String strSerialNumber, String strExtend, String strReturn) {
		// 验证签名
		String SignString = appendParam("", "uid", CustomerID);
		try {
			SignString = appendParam(SignString, "file", URLEncoder.encode(
					Base64.encode(strFilePath.getBytes("utf-8")), "utf-8"));
		} catch (Exception e) {
		}
		SignString = appendParam(SignString, "sn", strSerialNumber);
		
		try {
			SignString = appendParam(SignString, "ext", URLEncoder.encode(
					Base64.encode(strExtend.getBytes("utf-8")), "utf-8"));
		} catch (Exception e) {
		}
		SignString = appendParam(SignString, "ret", strReturn);
		
		String signcode = GetUploadSignCode(strFilePath, strSerialNumber,
				strExtend, strReturn);
	...
	public String GetProtectDownloadURLString(String FileName, int DiskID) {
		return GetProtectDownloadURLString(FileName, (short) 1, false, null,
				DiskID);
	}
	...
	public String GetProtectDownloadURLString(String FileName, Boolean IsDown,
			int DiskID) {
		if (IsDown)
			return GetProtectDownloadURLString(FileName, (short) 9, false,
					null, DiskID);
		else
			return GetProtectDownloadURLString(FileName, (short) 1, false,
					null, DiskID);
	}
	 ...
	 */
	public String GetProtectDownloadURLString(String FileName,
			short PlayerType, Boolean IsMultiFile, String MainFile) {
		return GetProtectDownloadURLString(FileName, PlayerType, IsMultiFile,
				MainFile, 1);
	}

	public String GetProtectDownloadURLString(String FileName,
			short PlayerType, Boolean IsMultiFile, String MainFile, int DiskID) {
		return getDownloadURLString("nor", FileName, PlayerType, IsMultiFile,
				MainFile, DiskID);
	}
	...
	public String GetProtectDownloadURLString(String FileName, Boolean IsDown,
			Boolean IsMultiFile, int DiskID) {
		if (IsDown)
			return GetProtectDownloadURLString(FileName, (short) 9,
					IsMultiFile, null, DiskID);
		else
			return GetProtectDownloadURLString(FileName, (short) 1,
					IsMultiFile, null, DiskID);
	}
	...
	public String GetEncryptDownloadURLString(String FileName, int DiskID) {
		return getDownloadURLString("dec", FileName, (short) 0, false, null,
				DiskID);
	}
	 ...
	 * @author BoatZhang
	 */
	public String GetEncryptDownloadURLString(String FileName, int PlayerType,
			Boolean IsMultiFile, String MainFile, int DiskID) {
		return getDownloadURLString("dec", FileName, (short) PlayerType,
				IsMultiFile, MainFile, DiskID);
	}

	public String GetEncryptDownloadURLString(String FileName, int PlayerType,
			Boolean IsMultiFile, String MainFile) {
		return GetEncryptDownloadURLString(FileName, PlayerType, IsMultiFile,
				MainFile, 1);
	}

	private String getDownloadURLString(String FileType, String FileName,
			short PlayerType, Boolean IsMultiFile, String MainFile, int DiskID) {
		try {
			String SerialNumber = "";

			String ApplySignCode = GetApplySignCode(FileName, CustomerID,
					CustomerKey);
			String ApplyURL = DownloadCenterURL + ApplyMethod
					+ GetApplyQueryString(FileName, ApplySignCode, CustomerID)
					+ "&disk=" + DiskID;
			...
			if (SerialNumber.length() > 0) {
				...
				if (IsMultiFile)
					return DownloadCenterURL
							+ FileType
							+ "/"
							+ GetFilenameSign(SerialNumber,
									String.valueOf(PlayerType)) + '/'
							+ MainFile;
				else
					return DownloadCenterURL
							+ FileType
							+ "/"
							+ GetFilenameSign(SerialNumber,
									String.valueOf(PlayerType)) + strExt;
	...
	public String GetFreeDownloadURLString(String FileName, Boolean IsDown,
		...
		String SignCode = GetSignCode(FileName, SerialNumber, CustomerID,
				CustomerKey);
		...
		return Result
				+ GetQueryString(FileName, SerialNumber, SignCode, CustomerID)
				+ "&disk=" + DiskID;
	}
	...
	public boolean TestFileExist(String FileName, int DiskID) {
		...
		String SignCode = GetSignCode(FileName, SerialNumber, CustomerID,
				CustomerKey);
		return "True"
				.equals(HttpUtil.PostHttpWebRequest(DownloadCenterURL
						+ GetMethod
						+ "check=1&disk="
						+ DiskID
						+ "&"
						+ GetQueryString(FileName, SerialNumber, SignCode,
								CustomerID)));
	}
	...
	public String GetFileInfo(String FileName, int DiskID) {
		...
		String ApplySignCode = GetApplySignCode("fileinfo", FileName,
				CustomerID, CustomerKey);
		String ApplyURL = DownloadCenterURL
				+ ApplyMethod
				+ GetApplyQueryString("fileinfo", FileName, ApplySignCode,
						CustomerID) + "&disk=" + DiskID;
	...
	public String GetResourcePlayURL(String FileName, Boolean IsMultiFile,
		...
		if (IsMultiFile)
			return DownloadCenterURL + PlayerPage + "uid=" + CustomerID
					+ "&sn=" + SerialNumber + "&multi=1";
		else
			return DownloadCenterURL + PlayerPage + "uid=" + CustomerID
					+ "&sn=" + SerialNumber + "&multi=0";
	}
	...
	public String GetExePackageURL(String FileName, String FilePath, int DiskID) {
		String sign = GetApplySignCode("exepackage", FileName, CustomerID,
				CustomerKey);
		try {
			return DownloadCenterURL + ApplyMethod
					+ GetApplyQueryString("exepackage", FileName, sign, CustomerID)
					+ "&path="+URLEncoder.encode(FilePath, "utf-8")+"&disk=" + DiskID;
		} catch (Exception e) {
			e.printStackTrace();
			return "";
		}
	...
	// 外部调用的公共方法 上传
	public String GetUploadURLString(String FilePath, String DownCenterURL,
			String Extend, String ReturnURL) {
		String SerialNumber = "";
		// 获取序列号，序列号20分钟（默认）内使用有效

		String ApplySignCode = GetApplySignCode(FilePath, CustomerID,
				CustomerKey);
		String ApplyURL = DownloadCenterURL + ApplyMethod
				+ GetApplyQueryString(FilePath, ApplySignCode, CustomerID);
		SerialNumber = HttpUtil.PostHttpWebRequest(ApplyURL);

		if (SerialNumber.length() > 0) {
			String Result = DownCenterURL + UploadMethod;
			String Code = GetUploadQueryString(FilePath, SerialNumber, Extend,
					ReturnURL);
			try {
	...
	public String GetUploadURLString(String FilePath, String Extend,
			String ReturnURL) {
		return GetUploadURLString(FilePath, DownloadCenterURL, Extend,
				ReturnURL);
	}
	...
	public boolean SendFileConvertTask(String FileName, String Extend, String ReturnURL)
    {
        ...
        String ApplyURL;
		try {
			ApplyURL = DownloadCenterURL + ApplyMethod + GetApplyQueryString("convert", FileName, sign, CustomerID) +
			    "&ext=" + URLEncoder.encode(Base64.encode(Extend.getBytes("utf-8")), "utf-8") + "&ret=" + URLEncoder.encode(ReturnURL, "utf-8");
...
//		System.out.println(res);
		...
		/*  上传文件
		String url = down.GetUploadURLString("upFile\\", "rename&uid=123&pp=中文", "");
		System.out.println(url);
		res = HttpUtil.PostFileToServer(url, "F:\\知好乐资源中心云部署方案.docx");
		System.out.println(res);
		*/
...
//		
		...
		src[0] = "upFile\\2015\\10105\\390320126\\2015121810495846198-28.gif";
		operate.setSourceFile(src);

		String[] dst = new String[1];
		dst[0] = "upFile\\2015\\one\\copy\\2015121810495846198-28.gif";
		operate.setDestFile(dst);

		Boolean result = down.SendFileOperateTask(operate);
		if (result)
		 ...
		 * src[0] = "GZ\\0101\\GZ0101011734\\GZ0101011734_QueQ.jpg"; src[1] =
		 * "GZ\\0101\\GZ0101011734\\GZ0101011734_SubQ_1.jpg"; src[2] =
		 * "GZ\\0101\\GZ0101011742\\新建文件夹\\a.txt"; src[3] =
		 * "GZ\\0101\\GZ0101011742\\GZ0101011742_QueQ.jpg";
		 * task.setSourceFile(src);
		 * 
		 * String[] ent = new String[4]; ent[0] =
		 * "GZ0101011734\\GZ0101011734_QueQ.jpg"; ent[1] =
		 * "GZ0101011734\\GZ0101011734_SubQ_1.jpg"; ent[2] =
		 * "GZ0101011742\\新建文件夹\\a.txt"; ent[3] =
		 * "GZ0101011742\\GZ0101011742_QueQ.jpg"; task.setEntryName(ent);
		 * 
		 * //JSONObject object5 = (JSONObject) JSONSerializer.toJSON(task);
		 * String json = JSONObject.fromObject(task).toString();
		 * 
		 * String CustomerID = "0000000000"; String CustomerKey = "1111111111";
		 * zhldowncenter down = new zhldowncenter(CustomerID, CustomerKey,
		 * "http://localhost/d/"); Boolean res = down.SendZipPackageTask(task);
		 * if (res) System.out.println("success"); else
		 * System.out.println(down.LastError);
		 * 
		 * System.out.println(down.GetFreeDownloadURLString("GZ\\1602.zip")); //
		 * task.NotifyUrl = "http://localhost/d/sss.aspx?zip=" +
		 * HttpUtility.UrlEncode(task.ZipFileName);
		 */
	}
}
