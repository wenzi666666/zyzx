$(function() {
	...
	$(".smallClassBox").height($(window).height());
	//videoList高度
	...
	if ($(window).height() < 470) {
		$(".videoInfo").height(470);
		$(".mb").height(490);
	} else {
		$(".videoInfo").height($(window).height() - 20);
	...
	window.onorientationchange = function() {
		...
		if (body.getAttribute("orient") == "landscape") {
			...
			setTimeout(function() {
				...
				$(".videoList li").height(($(".videoList").height() - 15) / 3.2);
	...
	$(".term").change(function() {
		...
		if ($(".term option:selected").attr("data-id") == "seven") {
			...
			$(".subject li[data-id='math']").css({
				"margin-left": ($(".subject").width() - $(".subject li").width()) / 2 + "px"
			});
		} else if ($(".term option:selected").attr("data-id") == "eight") {
			$(".subject li").show();
			$(".subject li[data-id='chemistry']").hide();
			$(".subject li[data-id='math']").css({
				"margin-left": ($(".subject").width() - $(".subject li").width()) / 3.4 + "px"
			});
		} else {
			$(".subject li").show();
			$(".subject li").css({
