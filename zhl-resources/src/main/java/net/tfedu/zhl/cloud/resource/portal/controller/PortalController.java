...
public class PortalController {
	 ...
	 * 教师动态（校级页面、区级页面通用）
	 * @param request
	 * @param tab   		页面类型：1 校级页面 2区级页面
	 * @param queryId       查询条件id， tab 为 1 时，为学校id；tab为2时，是地区id
	...
	public ResultJSON dynamic(HttpServletRequest request,int tab,long queryId) throws Exception{
		...
		switch(tab){
		case 1: return portalService.schoolDynamic(queryId);
		case 2: return portalService.distrcitDynamic(queryId);
	...
	@ResponseBody
	public ResultJSON resourceViewTop(HttpServletRequest request,long districtId) throws Exception{
