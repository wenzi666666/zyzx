...
public class UserCommentServiceTest extends BaseServiceTestCase {
	...
	public void testUserComment()throws IOException{
		...
		/*//查询其他人的评论
		List<UserComment> otherComments = userCommentService.getOtherComments(fromFlag, resId, userId);
		Assert.isTrue(otherComments.size() > 0);
		for(int i = 0;i < otherComments.size();i++)
			log.info(otherComments.get(i).toString());
		*/
		/*//增加
