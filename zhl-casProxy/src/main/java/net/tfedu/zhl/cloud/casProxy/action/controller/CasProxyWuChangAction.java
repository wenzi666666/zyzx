 ...
 * 
 * 
 * 武昌平台使用身份证号作为用户名，所以只能提供 用户的uuid 和  用户名称name。 
         其中用户名称会在BASE64编码之后传递过来
 ...
 */
@Controller
@RequestMapping("/casProxy/wuchangedu")
public class CasProxyWuChangAction {

	Logger log = LoggerFactory.getLogger(CasProxyWuChangAction.class);
	 ...
	 */
	@Resource
	ProxyService proxyServiceWuChang;
	...
	public void login(HttpServletRequest request, HttpServletResponse response) throws CustomException, Exception {
		...
		log.info("----parseAPI-----with----" + proxyServiceWuChang.getClass().getName());

		RegisterAddForm form = proxyServiceWuChang.parseAPI(request, casConfig);
