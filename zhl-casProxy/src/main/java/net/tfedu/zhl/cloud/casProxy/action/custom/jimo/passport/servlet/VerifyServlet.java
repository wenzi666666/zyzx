 ...
 * $Id: VerifyServlet.java 10707 2010-12-09 12:47:56Z huangwj $
 */
...
public class VerifyServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        if (StringUtils.isEmpty(input)) {
            if (!PassportClient.getInstance().isValidVerifyAuth(ticket, url, auth)) {
                print("Passport校验码非法", resp);
                return;
            }
        }
        else if (!PassportClient.getInstance().isValidVerifyAuth(ticket, url, input, auth)) {
            print("Passport校验码非法", resp);
            return;
        }
